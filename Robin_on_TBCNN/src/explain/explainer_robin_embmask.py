...
class TBCNN_explainer(BaseLayer):
    ...
    def explainer(self):
        #### selection
        _, self.sel_last_conv_output = self.convolve_tree_based_cnn_exp()
    ...
    def feed_forward(self):
        ...
        # train approximator
        self.pos_loss = self.s_loss + 3e-3 * self.us_loss
    ...
    def convolve_tree_based_cnn_exp(self):
        # explainer
        
        print("Including both type and token information..........")

        self.parent_node_type_embedding = self.compute_parent_node_types_tensor(self.placeholders["node_type"], self.weights_exp["node_type_embedding"])
        ...
        self.parent_node_token_embedding = self.compute_parent_node_tokens_tensor(self.placeholders["node_token"], self.weights_exp["node_token_embedding"])
                    
        # shape = (batch_size, max_tree_size, max_children, node_token_dim)
        # Example with batch size = 12: shape = (12, 48, 7, 50)
        self.children_node_token_embedding = self.compute_children_node_tokens_tensor(self.placeholders["children_node_token"], self.node_token_dim, self.weights_exp["node_token_embedding"])
        ...
        self.parent_node_embedding = tf.compat.v1.layers.dense(self.parent_node_embedding, units=self.node_dim, activation=tf.nn.leaky_relu, name='explainer_dense1')
        ...
        self.children_embedding = tf.compat.v1.layers.dense(self.children_embedding, units=self.node_dim, activation=tf.nn.leaky_relu, name='explainer_dense2')
        
        """Tree based Convolutional Layer"""
        # Example with batch size = 12 and num_conv = 8: shape = (12, 48, 128, 8)
        conv_output = self.conv_layer_exp(self.parent_node_embedding, self.children_embedding, self.placeholders["children_index"], self.num_conv, self.node_dim)
        return conv_output
    ...
    def convolve_tree_based_cnn_sapp(self):    
        ...
        '''
        # children_node_type
        children_node_type_shape = [basize * nodenum, -1]
        children_node_type = tf.reshape(self.mask_children_node_type_embedding, children_node_type_shape)
        children_node_type_smask = tf.tile(smask, multiples=[1,tf.shape(children_node_type)[1]])
        self.mask_children_node_type_embedding = tf.reshape(self.mul_grad(children_node_type, children_node_type_smask), tf.shape(self.mask_children_node_type_embedding))
        #self.mask_children_node_type_embedding = self.cast_grad(self.mask_children_node_type_embedding)
        # parent_node_token
        parent_node_token_shape = [basize * nodenum, -1]
        parent_node_token = tf.reshape(self.mask_parent_node_token_embedding, parent_node_token_shape)
        parent_node_token_smask = tf.tile(smask, multiples=[1,tf.shape(parent_node_token)[1]])
        self.mask_parent_node_token_embedding = tf.reshape(self.mul_grad(parent_node_token, parent_node_token_smask), tf.shape(self.mask_parent_node_token_embedding))
        #self.mask_parent_node_token_embedding = self.cast_grad(self.mask_parent_node_token_embedding)
        # children_node_token
        children_node_token_shape = [basize * nodenum, -1]
        children_node_token = tf.reshape(self.mask_children_node_token_embedding, children_node_token_shape)
        children_node_token_smask = tf.tile(smask, multiples=[1,tf.shape(children_node_token)[1]])
        self.mask_children_node_token_embedding = tf.reshape(self.mul_grad(children_node_token, children_node_token_smask), tf.shape(self.mask_children_node_token_embedding))
        #self.mask_children_node_token_embedding = self.cast_grad(self.mask_children_node_token_embedding)
        '''
    ...
    def convolve_tree_based_cnn_usapp(self):
        ...
        # children_node_type
        children_node_type_shape = [basize * nodenum, -1]
        children_node_type = tf.reshape(self.umask_children_node_type_embedding, children_node_type_shape)
        children_node_type_sumask = tf.tile(sumask, multiples=[1,tf.shape(children_node_type)[1]])
        self.umask_children_node_type_embedding = tf.reshape(self.mul_grad(children_node_type, children_node_type_sumask), tf.shape(self.umask_children_node_type_embedding))
        #self.umask_children_node_type_embedding = self.cast_grad(self.umask_children_node_type_embedding)
        # parent_node_token
        parent_node_token_shape = [basize * nodenum, -1]
        parent_node_token = tf.reshape(self.umask_parent_node_token_embedding, parent_node_token_shape)
        parent_node_token_sumask = tf.tile(sumask, multiples=[1,tf.shape(parent_node_token)[1]])
        self.umask_parent_node_token_embedding = tf.reshape(self.mul_grad(parent_node_token, parent_node_token_sumask), tf.shape(self.umask_parent_node_token_embedding))
        #self.umask_parent_node_token_embedding = self.cast_grad(self.umask_parent_node_token_embedding)
        # children_node_token
        children_node_token_shape = [basize * nodenum, -1]
        children_node_token = tf.reshape(self.umask_children_node_token_embedding, children_node_token_shape)
        children_node_token_sumask = tf.tile(sumask, multiples=[1,tf.shape(children_node_token)[1]])
        self.umask_children_node_token_embedding = tf.reshape(self.mul_grad(children_node_token, children_node_token_sumask), tf.shape(self.umask_children_node_token_embedding))
        #self.umask_children_node_token_embedding = self.cast_grad(self.umask_children_node_token_embedding)
        '''
    ...
    '''
    def sel_aggregation_layer(self, conv):
        # conv is (batch_size, max_tree_size, conv_output_dim)
        batch_size = tf.shape(conv)[0]
        max_tree_size = tf.shape(conv)[1]
        # conv_output_dim = tf.shape(conv)[2]
        contexts_sum = tf.reduce_sum(conv, axis=2, keepdims = True)
        # contexts_sum_average = tf.divide(contexts_sum, tf.cast(tf.expand_dims(conv_output_dim, -1), tf.float32))
        return contexts_sum
    '''
    ...
    def sel_aggregation_layer(self, conv):
        # conv is (batch_size, max_tree_size, conv_output_dim)
        conv = tf.compat.v1.layers.dense(conv, units=100, activation=tf.nn.leaky_relu)
        conv = tf.compat.v1.layers.dense(conv, units=100, activation=tf.nn.leaky_relu)
        conv = tf.compat.v1.layers.dense(conv, units=1, activation=tf.nn.leaky_relu)
        return conv
    '''
    ...
    '''
    def grad_Sample_Concrete(self, logits):
        logits_ = tf.compat.v1.keras.backend.permute_dimensions(logits, (0,2,1))# [batch_size, 1, d]
        batch_size = tf.shape(input=logits_)[0]
        d = tf.shape(input=logits_)[2]
        unif_shape = (batch_size, self.k, d)
    ...
    def ouput_Sample_Concrete(self, logits):
        """
        Layer for sample Concrete / Gumbel-Softmax variables. 
        ...
        logits = tf.reshape(logits, [-1, d]) 
        threshold = tf.expand_dims(tf.nn.top_k(logits, self.k, sorted = True)[0][:,-1], -1)
        discrete_logits = tf.cast(tf.greater_equal(logits,threshold), tf.int32)
        
        #output = tf.compat.v1.keras.backend.in_train_phase(samples, discrete_logits)
        output = tf.cast(discrete_logits, tf.float32)
        return output
        
    @tf.custom_gradient
    def Sample_Concrete(self, logits):
        def grad(dy):
            grads = tf.compat.v1.gradients(self.grad_Sample_Concrete(logits), logits)
            return grads
        output = self.ouput_Sample_Concrete(logits)
        return output, grad
    '''
    @tf.custom_gradient

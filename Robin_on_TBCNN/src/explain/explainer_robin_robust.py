...
class TBCNN_explainer(BaseLayer):
    ...
    def Cal_dif(self):
        '''
        #mask*input
        node_type = tf.cast(self.placeholders["node_type"], tf.float32) #[batch_szie, nodenum]
        dif_node_type = tf.cast(self.placeholders["dif_node_type"], tf.float32) #[batch_szie*dif_num, nodenum]

        bsize = tf.shape(node_type)[0]
        nodenum = tf.shape(node_type)[1]

        smask = self.mask
        mask_node_type = self.mul_grad(node_type, smask)

        dif_smask = tf.reshape(tf.tile(self.mask, [1, self.dif_num]), [bsize * self.dif_num, nodenum])
        mask_dif_node_type = self.mul_grad(dif_node_type, dif_smask)

        #cal dif
        mask_node_type = tf.reshape(tf.tile(mask_node_type, [1, self.dif_num]), [bsize*self.dif_num, nodenum])
        inter = self.inter_grad(mask_node_type, mask_dif_node_type)
        return tf.divide(inter, bsize*self.dif_num)
        '''
        ...
        return avg_sum
    '''
    def inter_grad(self, x, y):
        x = tf.cast(x, tf.int32)
        y = tf.cast(y, tf.int32)
        inter = tf.compat.v1.sets.set_intersection(x, y)
        interd = tf.sparse.to_dense(inter, default_value=0)
        inter = tf.cast(interd, tf.float32)

        gradx = tf.cast((tf.reduce_sum(tf.cast(tf.equal(x, tf.expand_dims(interd, axis=-1)), tf.int32), axis = 1) + tf.shape(inter)[1] - 1) // tf.shape(inter)[1], tf.float32)
        grady = tf.cast((tf.reduce_sum(tf.cast(tf.equal(y, tf.expand_dims(interd, axis=-1)), tf.int32), axis = 1) + tf.shape(inter)[1] - 1) // tf.shape(inter)[1], tf.float32)
        def grad(dy):
            #dx = tf.divide(interxd, xx)
            #dy = tf.divide(interyd, yy)
            dx = gradx
            dy = grady
            return dx, dy
        return inter, grad
    ...
    def feed_forward(self):
        ...
        self.neg_loss_dif = self.s_loss - 3e-3 * self.us_loss + 1e-4 * self.dif_loss

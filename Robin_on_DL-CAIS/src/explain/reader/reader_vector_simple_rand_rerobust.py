...
class Reader(threading.Thread):
    ...
    def __init__(self, x_train, y_train, difx_train, dif_num = 1, ratio_subset = 1., rng_seed=123, batch_size=32, flag_shuffle=True):
        """
        ...
        self.difx_train = difx_train
        self.dif_num = dif_num
        ...
        self.shuffle_index = self.gen_idx() 
    ...
    def run(self):
        ...
        while self.running:
            if self.data_buffer is None:
                ...
                redifx = self.difx_train.reshape(self.difx_train.shape[0]//self.dif_num, self.dif_num, self.difx_train.shape[1], self.difx_train.shape[2]) 
                difdata = redifx[batch_index]
                difdata = difdata.reshape(difdata.shape[0], difdata.shape[1]*difdata.shape[2], difdata.shape[3])
   

                with self.lock:
                    self.data_buffer = data, label, difdata
            sleep(0.0001)

...
from py import java_import,typedef,type_define,include
import attack, get_style
...
def scan_style(program_path, form_flag='best'):
    ...
    for root, sub_dirs, files in os.walk(program_path):
        for sub_dir in sub_dirs:
            ...
            dst_path = os.path.join('program_file/untargeted_attack_file', sub_dir)
...
def move_change(file_name, sub_dir, dst_aut):
    filename = file_name.replace(file_name.split('.')[-1], '')[:-1]
    change_name = 'mv ./style/transform/* ./style/transform/'+filename+'##'+sub_dir+'###'+dst_aut + \
                  '.'+file_name.split('.')[-1]
...
def get_input(path_program, file_name, src_name, form_flag):
    ...
    if form_flag == 'best':
        ...
        dst_aut = dst_aut.split('/')[-1].replace(dst_aut.split('/')[-1].split('.')[-1], '')[:-1]
        # convert the program to this target author
        attack.start_trans(path_program, dst_aut)
        # move each converted program to the final directory
        move_change(file_name, src_name, dst_aut)
        ...
        for author_name in author_list:
            if not author_name.endswith('.txt'):
                continue
            author_name = author_name.replace(author_name.split('.')[-1], '')[:-1]
            ...
            # move each converted program to the final directory
            move_change(file_name, src_name, dst_aut)
    elif form_flag == 'generate':
        # Generate author styles randomly
        # genrate
        for i in range(30):
            dst_aut = 'generate'
            # convert the program to this target author
            attack.start_trans(path_program, dst_aut)
            dst_aut = 'generate_' + str(i)
            # move each converted program to the final directory
            move_change(file_name, src_name, dst_aut)
    return dst_aut
...
def get_author(path_program, pre_name, style_path):
    ...
    for file in os.listdir(style_path):
        file_name = file.replace(file.split('.')[-1], '')[:-1]
